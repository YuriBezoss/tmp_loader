...
Write(uint32_t Address, uint32_t Size, uint8_t* buffer) {
    ...
    if (CSP_OSPI_WriteMemory((uint8_t*) buffer, (Address & (0x0fffffff)), Size) != HAL_OK) {
        HAL_SuspendTick();
        return LOADER_FAIL;
    }
...
SectorErase(uint32_t EraseStartAddress, uint32_t EraseEndAddress) {
    ...
    if (CSP_OSPI_EraseSector(EraseStartAddress, EraseEndAddress) != HAL_OK) {
        HAL_SuspendTick();
        return LOADER_FAIL;
    }
